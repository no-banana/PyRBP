...
def evaluateMLclassifers(features, folds, labels, file_path, shuffle):
...
def evaluateDLclassifers(features, folds, labels, file_path='', shuffle='True'):
    ...
    DL_classifiers = {'CNN':CNN, 'RNN':RNN, 'ResNet':ResNet, 'MLP':MLP}
    ...
    for clf_name in DL_classifiers:
        ...
        for train_index, test_index in cv.split(labels):
            ...
            if clf_name == 'MLP':
                ...
                DL_classifiers[clf_name].fit(x=train_X, y=train_Y, epochs=30, batch_size=64, verbose=1, shuffle=True, callbacks=callbacks,
